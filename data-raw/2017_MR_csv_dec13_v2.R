...
dat <- rawdat %>%
        ...
        dplyr::mutate(date = as.Date(date, "%Y%m%d"),
                      ...
                      sex = plyr::mapvalues(sex, from = c(0, 1, 2), to = c(NA, "M", "F")),
                      mat = as.logical(mat),
                      ad = as.logical(ad),
...
CH_ms <- dat17 %>%
  ...
  dplyr::mutate(ch = paste0(event1, event2, event3, event4, event5, event6)) %>%
  dplyr::select(-dplyr::starts_with("event"))
#saveRDS(CH_ms, ".\\data\\CH_ms17")

recap_rows <- sapply(strsplit(CH$ch, NULL), function(x) sum(as.numeric(x)))
table(recap_rows)
length(recaps)

dup_norecap
#In CH not marked recaps
setdiff(CH[recap_rows > 1, ]$tag, recaps)
...
same_event
setdiff(recaps, CH[recap_rows > 1, ]$tag) #in recaps not in CH (tagged in same event)

#55 recaps in CH
#60 in recaps
#60(recap) - 5(same event) = 55
# 
# marray <- function(CH){
#   nind <- dim(CH)[1]
